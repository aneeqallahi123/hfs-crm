  ...
  return (
    <Modal title="New engagement" onClose={onClose}>
      ...
      <div className="flex justify-end gap-2 pt-1">
        ...
        <Btn onClick={save} disabled={saving || yearTaken}>
...
export default function ClientDetail() {
  ...
  function statusColor(e) {
    if (e.deadline && e.deadline < today) return 'bg-rose-50 text-deep border-rose-200';
    if (e.deadline) return 'bg-fog text-green border-tint';
          ...
          {engagements.map(e => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="font-medium text-ink text-sm">FY {e.year} · {e.module}</div>
                <div className="text-xs text-slate-500 mt-0.5">
                  {e.incharge && <span>{e.incharge}</span>}
                  {e.deadline && <span>{e.incharge ? ' · ' : ''}Due {e.deadline}</span>}
                </div>
              </div>
